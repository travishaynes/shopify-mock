...
To enable / disable Mockify manually, set Mockify.enabled to true or false:

    Mockify.enabled = true # or false

And to completely disable access to the Internet altogether, or re-enable it:

    Mockify.allow_internet = false # or true to re-enable
...
You also have access to the Mockify fixtures from within your app to help with
testing. To access a fixture, use Mockify::Fixtures:

    Mockify::Fixtures.read(:orders) # => contents of orders.json

You can also use your own fixtures if you'd like:

    Mockify::Fixtures.use(:orders, order.to_json)

All the fixtures are copied directly from the {Shopify API}[http://api.shopify.com]
